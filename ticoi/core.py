...
def inversion_core(data: list, i: float | int, j: float | int, dates_range: np.ndarray | None = None, solver: str = 'LSMR',
    ...
    if data[0].size:  # If there are available data on this pixel
        ...
        Weightx = weight_for_inversion(weight_origine=apriori_weight, conf=conf, data=data_values, pos=2, inside_Tukey=False, temporal_decorrelation=weight_temporal_decorrelation)
        Weighty = weight_for_inversion(weight_origine=apriori_weight,  conf=conf, data=data_values, pos=3, inside_Tukey=False, temporal_decorrelation=weight_temporal_decorrelation)
...
def interpolation_core(result: pd.DataFrame, interval_output: int, path_save: str, option_interpol: str = 'spline',
    ...
    if first_date_interpol is not None and dataf_lp['date1'].iloc[0] > pd.Timestamp(first_date_interpol):
        ...
        nul_df = pd.DataFrame({'date1': first_date, 'date2': first_date + np.timedelta64(interval_output, 'D'),
    ...
    if last_date_interpol is not None and dataf_lp['date2'].iloc[-1] < pd.Timestamp(last_date_interpol):
        first_date = np.arange(dataf_lp['date2'].iloc[-1] + np.timedelta64(redundancy, 'D'), last_date_interpol,
                               np.timedelta64(redundancy, 'D'))
        nul_df = pd.DataFrame({'date1': first_date - np.timedelta64(interval_output, 'D'), 'date2': first_date,
             'vx': np.full(len(first_date), np.nan), 'vy': np.full(len(first_date), np.nan)})
    ...
    if 'invert' in returned or 'interp' in returned:
        ...
        if 'interp' in returned:   
            ...
            if result[1] is not None:  # If inversion have been performed
                ...
                if result_quality is not None and 'Norm_residual' in result_quality: 
                    returned_list.append(pd.DataFrame({'date1': [], 'date2': [], 'vx': [], 'vy': [],
    ...
    async def process_block(block: cube_data_class, returned: list | str = 'interp', nb_cpu: int = 8,
        ...
        if obs_filt is None and 'interp' in returned:
            if inversion_kwargs['result_quality'] is not None and 'Norm_residual' in inversion_kwargs['result_quality']:
                return [pd.DataFrame({'date1': [], 'date2': [], 'vx': [], 'vy': [], 'xcount_x': [], 'xcount_y': [], 'NormR': []})]
            else:
...
def visualization_core(list_dataf: pd.DataFrame, option_visual: List, save: bool = False, show: bool = True, path_save: Optional[str] = None, A: Optional[np.array] = None, log_scale: bool = False, cmap: str = 'rainbow', colors: List[str] = ['blueviolet', 'orange'],figsize:tuple[int, int] = (10,6)):
    ...
    pixel_object.load(list_dataf, save=save, show=show, A=A, path_save=path_save,figsize=figsize,type_data=['obs','invert'])
