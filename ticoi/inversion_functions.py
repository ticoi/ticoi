...
def construction_dates_range_np(data: np.ndarray) -> np.ndarray:
    ...
    :param data: an array where each line is (date1, date2, other elements) for which a velocity have been mesured
...
def studentized_residual(A: np.ndarray, residu: np.ndarray, dates_range: np.ndarray, W: np.ndarray, coef:int, regu:np.ndarray) -> np.ndarray:
    """
...
def externally_studentized_residual(A: np.ndarray, residu: np.ndarray, dates_range: np.ndarray,
                                    W: np.ndarray | None, coef: int, regu: np.ndarray) -> np.ndarray:
    """
...
@jit(nopython=True)
def matvecregu1_numba(X:np.ndarray, Y:np.ndarray, identification_obs:np.ndarray, delta:np.ndarray, coef:int, weight:np.ndarray):
...
def matrix_property(A:np.ndarray)->str:
    ...
    #                             INVERSION                                       #
    # =========================================================================%% #
def inversion_one_component(A:np.ndarray, dates_range:np.ndarray, v_pos:int, data:np.ndarray, solver:str, Weight:int|np.ndarray, mu:np.ndarray, coef:int=1, ini:None|np.ndarray=None, result_quality:None|list=None, regu:int|str=1, accel:None|np.ndarray=None,
                            linear_operator:"class_linear_operator"=None,
                            verbose:bool=False) -> (np.ndarray,np.ndarray|None):
    """
...
def inversion_two_components(A:np.ndarray, dates_range:np.ndarray, v_pos:int, data:np.ndarray, solver:str, Weight:int|np.ndarray, mu:np.ndarray, coef:int=1, ini:None|np.ndarray=None, show_L_curve:bool=False, verbose:bool=False):
    """
