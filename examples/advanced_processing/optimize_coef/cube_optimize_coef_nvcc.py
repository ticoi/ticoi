...
def plot_CVV(Coh_vector, title, vmax=1, mask_glacier=None, save=False):
    ...
    if mask_glacier is not None:
        Coh_vector = np.ma.masked_where(mask_glacier[::-1] == 0, Coh_vector)
...
def plot_diff_CVV(diff, name_save, mask_good_quality=None, mask_glacier=None, save=False):
    ...
    if mask_good_quality is not None:
        diff = np.where(mask_good_quality[::-1] > 1200, diff, np.nan)
    if mask_glacier is not None:
    ...
    if save:
        plt.savefig(f"{cube.filedir}/{cube.filename.split('.')[0]}_{name_save}_regu.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    fig1.savefig(f"{main_path}/Compa_VVC_coef")
    plt.show()
    ...
    fig1, ax1 = plt.subplots(figsize=(12, 4))
    ax1.plot(
        diff["param"], diff["diff"] * 100, linestyle="-", marker="o", color="orange", label=f"Velocity observations"
    )
    threshold = 0.025
    ax1.plot(
    ...
    plt.subplots_adjust(bottom=0.25)
    ax1.legend(loc="lower left", bbox_to_anchor=(0.12, -0.03), bbox_transform=fig1.transFigure, ncol=3, fontsize=16)
    fig1.savefig(f"{main_path}/Compa_VVC_coef_diff2")
    plt.show()
    ...
    plt.subplots_adjust(bottom=0.25)
    ax1.legend(loc="lower left", bbox_to_anchor=(0.12, -0.03), bbox_transform=fig1.transFigure, ncol=3, fontsize=16)
    fig1.savefig(f"{main_path}/Compa_VVC_coef_selectedlist")
    plt.show()
    ...
    plt.subplots_adjust(bottom=0.25)
    ax1.legend(loc="lower left", bbox_to_anchor=(0.12, -0.03), bbox_transform=fig1.transFigure, ncol=3, fontsize=16)
    fig1.savefig(f"{main_path}/Compa_VVC_coef_diff2_selectedlist")
    plt.show()
