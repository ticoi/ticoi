...
if save_mean_velocity and cube_interp is not None:
    mean_vv = (
        np.sqrt(cube_interp.ds["vx"].mean(dim="mid_date") ** 2 + cube_interp.ds["vy"].mean(dim="mid_date") ** 2)
        .to_numpy()
        .astype(np.float32)
    )
    ...
    dst_ds_temp = driver.Create(
        f"{path_save}{result_fn}_mean_velocity.tiff", mean_vv.shape[1], mean_vv.shape[0], 1, gdal.GDT_Float32
    )
    if TICOI_process != "load" or (TICOI_process == "load" and "raw" in cube_name.keys()):
        ...
        dst_ds_temp.SetGeoTransform(
            [np.min(cube_interp.ds["x"].values), resolution, 0, np.min(cube_interp.ds["y"].values), 0, resolution]
        )
...
result_tqdm = tqdm(useful_result, total=len(useful_result), mininterval=0.5)
