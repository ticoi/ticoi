...
# list_parameter = [150]
list_parameter = [50, 70, 90, 110, 130, 150, 170, 190, 210, 230, 250, 270, 290, 20, 40, 60, 80, 100,
                  120, 140, 160, 180, 200, 220, 240, 260, 280, 300, 500, 1000, 5000, 10000]
...
if interpolation: list_NCoh_vector_after_interpol = []
if zone_stable:
...
# %% For each parameter
f = open(f'{path_save}Paramaters_{options}_{parameter}.txt', 'w')
for param_value in list_parameter:
    ...
    if parameter == 'coef':
        ...
        raise NameError('Please enter as paramater "coef" or "interval_output"')
    ...
    if visual: visualisation(dataf, result, option_visual, save_path, A=A, dataf=dataf, unit=unit, show=True,
                             figsize=(12, 6))
    # Save the results
    if save: result.to_csv(f'{save_path}/ILF_result.csv')
    stop.append(time.time())
    ...
    dataf_lp = interpolation_core(result, path_save=save_path, data=dataf, first_date_interpol=start_date_interpol, 
    ...
    # # cube2 = cube_list[0][0].deepcopy()  # for comparison with orginal data
    ...
    if zone_stable:  # RMSE over stable areas
        ...
        Normalized_Coh_vector_after = np.sqrt(
            np.nansum(dataf_lp['vx'] / vv_after_inv) ** 2 + np.nansum(dataf_lp['vy'] / vv_after_inv) ** 2) / \
                                      dataf_lp.shape[0]
...
diff['param'][diff['diff'] < treshold / 100].iloc[0], diff['diff'][diff['diff'] < treshold / 100].iloc[0] * 100))
...
ax1.legend(loc='lower left', bbox_to_anchor=(0.12, -0.03), bbox_transform=fig1.transFigure, ncol=3, fontsize=16)
fig1.savefig(
    f'{path_save}/Compa_VVC_coef_diff2')
plt.show()
...
ax1.legend(loc='lower left', bbox_to_anchor=(0.12, -0.03), bbox_transform=fig1.transFigure, ncol=3, fontsize=16)
fig1.savefig(
    f'{path_save}/Compa_VVC_coef_selectedlist')
plt.show()
...
ax1.legend(loc='lower left', bbox_to_anchor=(0.12, -0.03), bbox_transform=fig1.transFigure, ncol=3, fontsize=16)
fig1.savefig(
    f'{path_save}/Compa_VVC_coef_diff2_selectedlist')
plt.show()

#
# # %%
# if zone_stable:
#     # Vizualisation of the original error x and y according to the temporal baseline
#     fig1, ax1 = plt.subplots(figsize=(12, 4))
