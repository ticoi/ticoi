...
save_mean_velocity = False # Save a .tiff file with the mean reulting velocities, as an example

## ------------------------------ Data selection --------------------------- ##
# List of the paths where the data cubes are stored
cube_names = ['/media/tristan/Data3/OIC_2024/S2/TICOI_inversion_2nd/Mont_Blac_displacement_S2.nc']
flag_shp = '/media/tristan/Data3/OIC_2024/S2/Alpes_S2_RGI7.shp'  # shpfile for generating the flag file
mask_file = None # Path where the mask file is stored
path_save = '/media/tristan/Data3/OIC_2024/S2/TICOI_inversion_2nd/' # Path where to store the results
result_fn = 'Mont_Blac_velocity_S2_median_filt_1accelnotnull'# Name of the netCDF file to be created
...
load_kwargs = {'chunks': {}, 
               ...
               'subset': None, #[353300, 354200, 5099600, 5101720], # Subset of the data to be loaded ([xmin, xmax, ymin, ymax] or None)
               'buffer': None, # Area to be loaded around the pixel ([longitude, latitude, buffer size] or None)
               'pick_date': None, # ['2015-08-29', '2023-10-11'], # Select dates ([min, max] or None to select all)
...
if TICOI_process == 'block_process':
    result = process_blocks_refine(cube, nb_cpu=nb_cpu, block_size=block_size, returned='invert', preData_kwargs=preData_kwargs, inversion_kwargs=inversion_kwargs)
...
if inversion_kwargs['interpolation']:
    ...
    cubenew = cube.write_result_tico(result, source, sensor, filename=result_fn, savepath=path_save if save else None, 
                                  result_quality=inversion_kwargs['result_quality'], verbose=inversion_kwargs['verbose'])
